...
__global__ void updateDownKernel(int n, int k, int h, float* A)
{
	...
	//if (blockIdx.x == 0) printSubMatrix(B, m, m, k); __syncthreads();
	//if (blockIdx.x == 0) printSubMatrix(C, m, m, k); __syncthreads();
	//if (blockIdx.x == 0) printSubMatrix(D, m, m, k); __syncthreads();
	
	if (threadIdx.x < Cm && threadIdx.y < Dn)
	{
		float tmp = 0;
		for (int i = 0; i < k; i++)
			tmp += C[i*k + threadIdx.x]*D[threadIdx.y*k + i];
			
		B[threadIdx.y*k + threadIdx.x] -= tmp;
	}
